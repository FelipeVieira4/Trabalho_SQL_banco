...
select crime.id,crime.tipoCrime,count(suspeito.id) as "Suspeitos" from crime_suspeito inner join suspeito on suspeito.id=crime_suspeito.id_suspeito inner join crime on crime.id = crime_suspeito.id_crime group by crime.id;
